...
This starter kit is a Laravel + Bootstrap application with Blade components and jQuery helpers. Keep changes consistent with the existing component and helper patterns.
...
$('#categories-table').jpDataTable({
    ...
    mobileCards: {
        ...
        breakpoint: 768,
        pageLength: 8,
        renderCard: row => `<div class="jp-mobile-card">${row.name}</div>`
    }
});
```
...
- Categories are the sample CRUD module and should stay useful as a reference implementation.
- The controller supports:
  ...
  - CSV export
  - Select2 API options
- Keep feature tests updated when changing this module.
...
- Add or update tests when changing routes, controllers, or important component behavior.
- Current smoke coverage includes dashboard, UI kit, categories bulk actions, category datatable requests, and Select2 API options.
